...
TEST(one_band_semicircular, half_filling) { // NOLINT
  ...
  std::vector<std::string> spin_names = {"up","dn"};
  auto gf_struct = gf_struct_t{{"up", {1}}, {"dn", {1}}};
  auto fops_local = fundamental_operator_set(gf_struct);
  
  double U = 1.5;
  auto h_loc = U * n("up", 1) * n("dn", 1);
  double mu = U / 2.0; // half-filling
 
  double half_bandwidth = 2.0 * t;
  int epts = 4096; 
  auto [dispersion, dos, de] = build_semicircular_dos(half_bandwidth, epts); // same dispersion for each block
  ...
  for (auto const cycle : range(num_cycles)) { 
    ...
    for (auto i : range(spin_names.size())) {
      ...
      auto h_qp = get_h_qp<double>({R_b}, {lambda_b}, dispersion, mu);
      auto disp_R = get_disp_R<double,double>({R_b},dispersion,h_qp);
    
      // 'Fake' k-sum -> energy integral
      auto wks = fermi_fnc(h_qp.val, beta) * dos * de;
  ...
  matrix<double> R_expected = {{{0.877819}}};
  matrix<double> lambda_expected = {{{mu_expected}}};

  EXPECT_NEAR(mu_expected, mu_calculated, 1e-5); // NOLINT
  for (auto block : spin_names) {
    EXPECT_ARRAY_NEAR(R_expected, R.at(block), 1e-5); // NOLINT
    EXPECT_ARRAY_NEAR(lambda_expected, lambda.at(block), 1e-5); // NOLINE
  }
}
