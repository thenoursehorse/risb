...
class oneshotRISB(object):
    def __init__(self, filename, 
                       dft_solver='vasp', root_solver='diis', sc_method='recursion', 
                       prnt=True, plo_cfg_filename='plo.cfg', analyze_block_threshold=1e-2, use_rotations=False,
                       beta=5.0, smearing='gaussian', mu_tol=1e-6,
    ...
    def one_dmft_cycle(self, Lambda, R):
        ...
        # Ensure symmetries are enforced
        for ish in range(self.SK.n_inequiv_shells):
        ...
        self.disablePrint()
        mu = self.SK.calc_mu_risb(precision = self.mu_tol)
        self.SK.chemical_potential = mpi.bcast(self.SK.chemical_potential)
        self.enablePrint()
        mpi.report("mu =", mu)
        ...
        for ish in range(self.SK.n_inequiv_shells):
            ...
            print("Solving U =", self.U, "J =", self.J, "impurity for shell", ish, "using", self.root_solver, "with", self.sc_method)
    ...
    def solve_diis(self, Lambda, R, alpha=1., N=3, C_inv=None, crop=False):
        if crop:
    ...
    # FIXME add way to set R and Lambda from previous files
    def sumk_setup(self):
        if not self.prnt:
            self.disablePrint()
        ...
        if self.dft_solver == 'vasp':
            Converter = vaspconv.VaspConverter(filename = self.filename)
        elif self.dft_solver == 'vasp-hk':
            Converter = vaspconv.VaspConverter(filename = self.filename, proj_or_hk = 'hk')
        elif self.dft_solver == 'wien2k':
            Converter = wien2kconv.Wien2kConverter(filename = self.filename)
        else:
        ...
        self.SK = SumkDFTRISB(hdf_file = self.filename+'.h5', use_dft_blocks = False,
                              beta=self.beta, smearing=self.smearing)

        # Analyze the structure to get the blocks for the local Green's function 
        ...
        for ish in range(len(self.SK.deg_shells)):
            ...
            for iblock in range(num_block_deg_orbs):
                ...
                for keys in list(self.SK.deg_shells[ish][iblock]): # for no gf
                    mpi.report('  '+keys)
        ...
        #    num_block_deg_orbs = len(SK.deg_shells[ish])
        #    mpi.report('found {0:d} blocks of degenerate orbitals in shell {1:d}'.format(num_block_deg_orbs, ish))
        #    for iblock in range(num_block_deg_orbs):
        #        mpi.report('block {0:d} consists of orbitals:'.format(iblock))
        #        for keys in list(SK.deg_shells[ish][iblock].keys()):
        #            mpi.report('  '+keys)
        ...
        # FIXME add optinos to remove orbitals from the projectors
        ...
        _, _ = self.SK.initialize_R_Lambda(random=True, zero_Lambda=False)
    ...
    def dmft_cycle(self, reset_sumk=False):
        if reset_sumk:
            self.sumk_setup()
        ...
        self.root_tol = 5e-2
        Lambda, R, _ = self.solve_recursion(Lambda=self.SK.Lambda, R=self.SK.R)
        ...
        ## In case SK and S last evaluation was not at convergence
        self.h_dc = self.get_h_dc()
        ...
        self.corr_energy = []
        for ish in range(self.SK.n_inequiv_shells):
            self.corr_energy.append( self.S[ish].overlap(self.h_int[ish]) )
        ...
        for icrsh in range(self.SK.n_inequiv_shells):
            ...
            self.total_dc_energy = self.dc_energy[icrsh]
                
        self.dft_energy = self.get_dft_energy()
        self.total_energy = self.dft_energy + self.band_en_correction
        ...
        ##mpi.report(f'Vdc: {Vdc}')
        for ish in range(self.SK.n_inequiv_shells):
        #    #mpi.report(f'h0_loc_mat: {h0_loc_mat}')
            mpi.report(f'Z: {self.S[ish].Z}')
        #    mpi.report(f'R: {self.S[ish].R}')
    ...
    def save(self, it=0, folder='data/'):
        ...
        if mpi.is_master_node():
            ...
            #if 'iteration_count' in ar['RISB_results']:
            #    it = ar['RISB_results']['iteration_count']+1
                
                # Update Lambda and R in the SumK class
                #SK.Lambda[0] = ar['RISB_results']['Iterations']['Lambda_it'+str(it-1)]
                #SK.R[0] = ar['RISB_results']['Iterations']['R_it'+str(it-1)]
                
                #SK.dc_imp = ar['RISB_results']['Iterations']['dc_imp_it'+str(it-1)]
                #SK.dc_energ = ar['RISB_results']['Iterations']['dc_energ_it'+str(it-1)]
                #SK.chemical_potential = ar['RISB_results']['Iterations']['chemical_potential_it'+str(it-1)].real
