...
from scipy.linalg import sqrtm  # FIXME REMOVE WHEN f1,f2 in sc
...
# FIXME REMOVE WHEN f1,f2 in sc
def get_f1(Mcf,pdensity,R):
    K = pdensity - numpy.dot(pdensity,pdensity)
    K_sq = sqrtm(K)
    return Mcf - numpy.dot(R.T, K_sq)
    #return logm(Mcf) - logm(numpy.dot(R.T, K_sq))

def get_f2(Nf, pdensity):
    return Nf - pdensity.T
    #return logm(Nf) - logm(pdensity.T)

class risbContainer(object):
    ...
    def one_cycle(self, SK, Lambda=None, R=None, sc_method='recursion'):
        ...
        self.emb_solver.set_h_emb(self.Lambda_c, self.D)
        ...
        for block in self.block_names:
            ...
            if (sc_method == 'recursion') or (sc_method == 'fixed-point'):
                ...
                out_A[block] = get_f1(self.Mcf[block], self.pdensity[block], self.R[block])
                out_B[block] = get_f2(self.Nf[block], self.pdensity[block])
            else:
